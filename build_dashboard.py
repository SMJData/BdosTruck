...
<div class="page">
  ...
  <main class="wrap">
    ...
    <section class="map-layout">
      <article class="card map-card">
        ...
        <div class="map-legend" aria-label="Map legend">
          ...
          <div class="legend-row"><span class="legend-dot"></span><span>Precise stop</span></div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th class="num">Day stop h</th><th class="num">Max speed</th><th>Zones</th>
            </tr>
          </thead>
    ...
    <p class="footer">Prepared from the active CSV. Day stop hours exclude gaps that continue into the next calendar day. Boundary: SimpleMaps Barbados WGS84 GeoJSON, Creative Commons Attribution 4.0. Road context: Caribbean Marine Atlas road network. Locality-anchor coordinates should be validated before operational routing decisions.</p>
  </main>
</div>
...
const CSV_FIELDS = {
  ...
  start: ["Start Date", "TripDetailStartDateTime"],
  stop: ["Stop Date", "TripDetailStopDateTime"],
  distance: ["Distance", "TripDetailDistance"],
  driving: ["Driving Duration", "TripDetailDrivingDuraion", "TripDetailDrivingDuration"],
  stopDuration: ["Stop Duration", "TripDetailStopDuration"],
  origin: ["Origin", "TripDetailStartLocation"],
  destination: ["Destination", "TripDetailLocation"],
  zoneTypes: ["Stop Zone Types", "Location.ZoneZoneTypes"],
  idling: ["Idling Duration", "TripDetailIdlingDuration"],
  maxSpeed: ["Maximum Speed", "TripDetailMaximumSpeed"],
  destinationLat: ["TripDetailLatitude"],
  destinationLon: ["TripDetailLongitude"]
};
const REQUIRED_CSV_FIELDS = ["device", "start", "stop", "distance", "driving", "stopDuration", "origin", "destination", "idling", "maxSpeed"];

function resolveCsvColumns(sample) {
  const columns = {};
  Object.entries(CSV_FIELDS).forEach(([key, aliases]) => {
    columns[key] = aliases.find(alias => Object.prototype.hasOwnProperty.call(sample, alias)) || null;
  });
  const missing = REQUIRED_CSV_FIELDS.filter(key => !columns[key]).map(key => CSV_FIELDS[key].join(" / "));
  if (missing.length) throw new Error(`CSV is missing required columns: ${missing.join(", ")}`);
  return columns;
}
function csvValue(row, columns, key) {
  return columns[key] ? row[columns[key]] : "";
}
...
function parseDateTime(value) {
  ...
  if (/^-?\d+(?:\.\d+)?$/.test(text)) return excelDate(text);
  const match = text.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})\s+(\d{1,2}):(\d{2})(?::(\d{2}))?\s*(AM|PM)?$/i);
  if (!match) throw new Error(`Unsupported date/time value: ${text || "blank"}`);
  let [, day, month, year, hour, minute, second = "0", meridiem] = match;
  let hourValue = Number(hour);
  if (meridiem) {
    hourValue %= 12;
    if (meridiem.toUpperCase() === "PM") hourValue += 12;
  }
  const date = new Date(Number(year), Number(month) - 1, Number(day), hourValue, Number(minute), Number(second));
  if (
    date.getFullYear() !== Number(year) || date.getMonth() !== Number(month) - 1 ||
    date.getDate() !== Number(day) || date.getHours() !== hourValue || date.getMinutes() !== Number(minute)
  ) throw new Error(`Invalid date/time value: ${text}`);
  return date;
}
...
function dateTimeLabel(date) {
  return `${dateKey(date)} ${date.toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit" })}`;
}
function durationHours(value) {
  ...
  if (text.includes(":")) {
    ...
    return hoursValue + minutesValue / 60 + secondsValue / 3600;
  }
  const number = Number(text);
  if (!Number.isFinite(number)) throw new Error(`Unsupported duration value: ${text}`);
  return number * 24;
}
function shortPlace(address) {
  ...
  if (text.includes(":")) {
    ...
    const namedPrefix = text.slice(0, colon).split(",")
      .map(part => part.trim())
      .filter(part => part && !/^\d+$/.test(part) && part.toLowerCase() !== "barbados" && !PLUS_RE.test(part))
      .join(", ");
    if (namedPrefix) return namedPrefix;
    text = text.slice(colon + 1);
  }
  text = text.replace(/\b\d{5,6},\s*/g, "").replace(PLUS_RE, "").replace(/,\s*Barbados/gi, "").replace(/\s*,\s*,+/g, ", ");
...
function buildDataset(csvText, sourceName) {
  ...
  const columns = resolveCsvColumns(parsedRows[0]);
  const rawRows = parsedRows.map((row, index) => ({
    sourceRow: index + 2,
    device: csvValue(row, columns, "device"),
    startValue: csvValue(row, columns, "start"),
    stopValue: csvValue(row, columns, "stop"),
    distanceValue: csvValue(row, columns, "distance"),
    drivingValue: csvValue(row, columns, "driving"),
    stopDurationValue: csvValue(row, columns, "stopDuration"),
    origin: csvValue(row, columns, "origin"),
    destination: csvValue(row, columns, "destination"),
    zoneTypes: csvValue(row, columns, "zoneTypes"),
    idlingValue: csvValue(row, columns, "idling"),
    maxSpeedValue: csvValue(row, columns, "maxSpeed"),
    destinationLatValue: csvValue(row, columns, "destinationLat"),
    destinationLonValue: csvValue(row, columns, "destinationLon")
  })).filter(row => row.startValue && row.stopValue && row.origin && row.destination);
  if (!rawRows.length) throw new Error("CSV contains no complete trip rows");
  const addressSet = new Set();
  const coordinateHints = {};
  rawRows.forEach(row => {
    addressSet.add(row.origin);
    addressSet.add(row.destination);
    const lat = Number(row.destinationLatValue), lon = Number(row.destinationLonValue);
    if (Number.isFinite(lat) && Number.isFinite(lon) && lat >= -90 && lat <= 90 && lon >= -180 && lon <= 180) {
      coordinateHints[row.destination] = { lat, lon, confidence: "precise", method: "CSV destination coordinate" };
    }
  });
  const locations = {};
  [...addressSet].sort().forEach(address => {
    locations[address] = { name: shortPlace(address), address, ...(coordinateHints[address] || coordinateFor(address)) };
  });
  const rows = rawRows.map((row, index) => {
    let start, stop;
    try {
      start = parseDateTime(row.startValue);
      stop = parseDateTime(row.stopValue);
    } catch (error) {
      throw new Error(`CSV row ${row.sourceRow}: ${error.message}`);
    }
    if (stop < start) throw new Error(`CSV row ${row.sourceRow}: stop time is before start time`);
    const origin = locations[row.origin];
    const destination = locations[row.destination];
    const sourceDrivingHours = durationHours(row.drivingValue);
    const sourceStopHours = durationHours(row.stopDurationValue);
    const usesRoundedClockDurations = String(row.drivingValue).includes(":");
    return {
      id: index + 1,
      device: row.device,
      date: dateKey(start),
      ...
      distance: Number(row.distanceValue || 0),
      drivingHours: usesRoundedClockDurations ? (stop - start) / 3600000 : sourceDrivingHours,
      stopHours: sourceStopHours,
      idlingHours: durationHours(row.idlingValue),
      maxSpeed: Number(row.maxSpeedValue || 0),
      zoneTypes: row.zoneTypes || "Unclassified",
      nearZero: Number(row.distanceValue || 0) < 0.05,
      _startMs: start.getTime(),
      _stopMs: stop.getTime(),
      _sourceStopHours: sourceStopHours,
      _usesRoundedClockDurations: usesRoundedClockDurations
    };
  }).sort((a, b) => a._startMs - b._startMs);
  const nextStartByDevice = new Map();
  for (let index = rows.length - 1; index >= 0; index--) {
    const row = rows[index];
    const nextStart = nextStartByDevice.get(row.device);
    const gapHours = nextStart === undefined ? NaN : (nextStart - row._stopMs) / 3600000;
    const hasNextTrip = Number.isFinite(gapHours) && gapHours >= 0;
    const fullStopHours = row._usesRoundedClockDurations && hasNextTrip ? gapHours : row._sourceStopHours;
    const stopEndMs = hasNextTrip ? nextStart : row._stopMs + fullStopHours * 3600000;
    row.isOvernightStop = dateKey(new Date(row._stopMs)) !== dateKey(new Date(stopEndMs));
    row.fullStopHours = fullStopHours;
    row.stopHours = row.isOvernightStop ? 0 : fullStopHours;
    nextStartByDevice.set(row.device, row._startMs);
  }
  ...
  const daily = days.map(date => {
    ...
    return {
      ...
      overnightStops: dayRows.filter(row => row.isOvernightStop).length,
  ...
  return {
    ...
    summary: {
      ...
      overnightStopCount: rows.filter(row => row.isOvernightStop).length,
...
function currentStats() {
  ...
  return {
    ...
    overnightStops: rows.filter(row => row.isOvernightStop).length,
...
function renderDetail(stop = null) {
  ...
  if (!stop) {
    ...
    panel.innerHTML = `
      ...
      <div class="detail-row"><span>Day stops</span><strong>${fmt(stats.stop, 2)} hours</strong></div>
      <div class="detail-row"><span>Overnight gaps</span><strong>${stats.overnightStops} excluded</strong></div>
...
function renderInsights() {
  ...
  byId("confidenceInsight").textContent = `${s.confidenceCounts.precise || 0} locations use CSV coordinates or Plus Codes; ${s.confidenceCounts.locality || 0} use named locality anchors and should be validated for operational routing.`;
}
function renderTable() {
  const rows = filteredRows();
  const overnightStops = rows.filter(row => row.isOvernightStop).length;
  byId("tableSubhead").textContent = `${rows.length} records shown from ${DATA.sourceName}; ${overnightStops} overnight gaps excluded from stop hours`;
  byId("tripRows").innerHTML = rows.map(row => `
    <tr class="${row.nearZero ? "zero" : ""}">
      ...
      <td class="num" data-label="Idle h">${fmt(row.idlingHours, 2)}</td><td class="num" data-label="Day stop h" title="${row.isOvernightStop ? "Overnight gap excluded" : ""}">${fmt(row.stopHours, 2)}</td>
