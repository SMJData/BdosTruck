...
@media (max-width: 760px) {
  ...
  .summary-grid, .dashboard-grid, .risk-grid { grid-template-columns: 1fr; }
  ...
  .chip, .btn, .file-btn { flex: 1 1 auto; justify-content: center; }
}
</style>
</head>
<body>
  ...
  <main class="wrap">
    <section class="source-row" aria-label="Data source controls">
      ...
      <div class="control-group">
        <button class="btn primary" type="button" id="refreshCsv">Refresh ServiceTruckCSV.csv</button>
        <label class="file-btn">Choose CSV<input type="file" id="csvFile" accept=".csv,text/csv"></label>
      </div>
    </section>
...
async function refreshCsv() {
  try {
    ...
    setStatus(`Could not load ServiceTruckCSV.csv (${error.message}). Keep it beside the HTML or use Choose CSV.`, true);
  }
}
...
function renderTable() {
  ...
  byId("tripRows").innerHTML = rows.map(row => `
    <tr class="${row.nearZero ? "zero" : ""}">
      <td>${esc(row.start)}</td><td>${esc(row.origin)}</td><td>${esc(row.destination)}</td>
      <td class="num">${fmt(row.distance, 2)}</td><td class="num">${fmt(row.drivingHours, 2)}</td>
      <td class="num">${fmt(row.idlingHours, 2)}</td><td class="num">${fmt(row.stopHours, 2)}</td>
      <td class="num">${fmtCompact(row.maxSpeed)}</td><td>${esc(row.zoneTypes)}</td>
    </tr>`).join("");
}
function renderAll() {
  renderFilters();
...
byId("refreshCsv").addEventListener("click", refreshCsv);
